...
const AnimatedStars = () => {
  ...
  useEffect(() => {
    ...
    const makeStar = () => {
      ...
      const radius =
        r < 0.6
          ? Math.random() * 0.7 + 0.2 // pequenas
          : r < 0.9
            ? Math.random() * 0.6 + 0.9 // médias
      ...
      return {
        ...
        baseAlpha:
          radius > 1.4
            ? 0.6 + Math.random() * 0.4 // brilhantes
            : radius > 0.9
    ...
    const draw = (ts: number) => {
      ...
      for (const s of stars) {
        const alpha = Math.max(
          0,
          s.baseAlpha + s.twinkleAmp * Math.sin(t * s.twinkleFreq * Math.PI * 2 + s.twinklePhase)
        );
