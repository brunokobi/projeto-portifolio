...
describe("speedToColor", () => {
  it("vento calmo (0 m/s) fica na ponta azul da escala", () => {
    expect(speedToColor(0)).toBe("rgb(36,104,180)");
  });

  it("vento forte (>= topo da escala) fica na ponta vermelha", () => {
    expect(speedToColor(100)).toBe("rgb(215,40,40)");
  });

  it("velocidades negativas são tratadas como zero", () => {
    expect(speedToColor(-5)).toBe(speedToColor(0));
  });

  it("é monotonicamente crescente em pelo menos um canal entre paradas", () => {
